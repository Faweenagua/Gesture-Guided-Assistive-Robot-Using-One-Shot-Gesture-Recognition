...
class GestureRecognitionNode(Node):
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter("language_path", "/home/francis/IRS_ws/src/taraBot/gestures.json")
        self.declare_parameter("start_pose_threshold", 30)
        self.declare_parameter("stop_pose_threshold", 30)

        language_path = self.get_parameter("language_path").value
        self.START_POSE_THRESHOLD = self.get_parameter("start_pose_threshold").value
        self.STOP_POSE_THRESHOLD = self.get_parameter("stop_pose_threshold").value

        # Load gesture recognizer
        self.recognizer = GestureRecognizer(language_path=language_path,
                                            num_clusters=10,
                                            random_state=10)
        ...
        # Publisher for recognized gesture
        self.result_pub = self.create_publisher(String, "/gesture_recognition/result", 10)
        ...
        self.get_logger().info("Gesture Recognition Node started (Option 2: Webcam inside node).")

    # -------------------------------------------------------------
    # Helper for selecting largest hand
    # -------------------------------------------------------------
    def prioritize_hand_by_bounding_box(self, hand_results):
        def get_bbox_size(hand_lms):
            xs = [lm.x for lm in hand_lms.landmark]
            ys = [lm.y for lm in hand_lms.landmark]
            return (max(xs) - min(xs)) * (max(ys) - min(ys))

        sizes = [(i, get_bbox_size(hand))
                 for i, hand in enumerate(hand_results.multi_hand_landmarks)]

        idx = max(sizes, key=lambda x: x[1])[0]
        return (hand_results.multi_hand_landmarks[idx],
                hand_results.multi_handedness[idx].classification[0].label)

    # -------------------------------------------------------------
    # Main loop: Read webcam, process frame
    # -------------------------------------------------------------
    def timer_callback(self):
        ...
        if pose_res.pose_landmarks:
            ...
            if hand_res.multi_hand_landmarks:
                prioritized, label = self.prioritize_hand_by_bounding_box(hand_res)
                landmarks = {i: lm for i, lm in enumerate(prioritized.landmark)}
                vulcan = is_vulcan_salute(landmarks)
            ...
            if self.state == "IDLE":
                if self.start_pose_frames >= self.START_POSE_THRESHOLD:
                    ...
                    self.start_pose_frames = 0
                    self.stop_pose_frames = 0
                    self.get_logger().info("Recording gesture...")

            elif self.state == "RECORDING":
                kp = {
                    mp.solutions.pose.PoseLandmark(i).name: {"x": lm.x, "y": lm.y}
                    for i, lm in enumerate(pose_res.pose_landmarks.landmark)
                }
                self.collected_keypoints.append(kp)
                ...
                gesture = self.recognizer.identify_gesture(self.collected_keypoints)
                self.get_logger().info(f"Recognized gesture: {gesture}")
                self.result_pub.publish(String(data=gesture))
        ...
        # Close window when ESC or 'q' is pressed
        if key == 27 or key == ord('q'):
            self.get_logger().info("Exit key pressed. Closing webcam window...")
            rclpy.shutdown()
            return

    # -------------------------------------------------------------
    def destroy_node(self):
        self.get_logger().info("Shutting down gesture node.")
        self.cap.release()
